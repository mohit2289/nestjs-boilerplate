/**
 * @author Mohit Verma
 * @description subcategory.validator is validate api payload data
 * */
import {
  HttpException,
  HttpStatus,
...
@Injectable()
export class SubCategoryValidator implements NestMiddleware {
  ...
  addSubcategoryValid = Joi.object().keys({
    name: Joi.string()
      .required()
      .error(new Error(SubcategoryErrMessageConstant.nameCheck)),
    ...
    parentCategoryId: Joi.string()
      .required()
      .regex(this.onlyNum)
      .error(new Error(SubcategoryErrMessageConstant.parentCategoryId)),
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    const { error } = this.addSubcategoryValid.validate(req.body);
    if (error) {
      if (Object.entries(req.files).length > 0) {
